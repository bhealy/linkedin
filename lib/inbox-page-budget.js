...
function blockReason(snapshot) {
  ...
  if (snapshot.lastHour >= snapshot.hourLimit) {
    parts.push(
      `${snapshot.lastHour.toLocaleString()} LinkedIn pages in the last hour (limit ${snapshot.hourLimit.toLocaleString()})`
    );
  }
  if (snapshot.lastFourHours >= snapshot.fourHourLimit) {
    parts.push(
      `${snapshot.lastFourHours.toLocaleString()} LinkedIn pages in the last 4 hours (limit ${snapshot.fourHourLimit.toLocaleString()})`
    );
  }
