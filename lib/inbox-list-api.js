...
function classifyEmptyListPage({ startCursor, savedCursor } = {}) {
  if (!startCursor) {
    return { staleCursor: false, stopReason: 'reached the end of the inbox' };
  }
  if (savedCursor) {
    return {
      staleCursor: true,
  ...
  // A cursor derived from the oldest cached row is not a LinkedIn resume
  // token. An empty first page means the cache already reached the end.
  return { staleCursor: false, stopReason: 'reached the end of the inbox' };
}
